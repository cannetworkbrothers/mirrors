...
class SocketServer(object):
    ...
    def listen(self, buffer_size):
        ...
        all_messages_file = open(config.NEW_CAN_MESSAGES_FILE_NAME, "w")
        ...
        while True:
            ...
            while True:
                ...
                if valid_str:
                    #starts on W,S
                    is_msg_present, current_str, recent_msg = com_parser.get_messages(current_str)
                else:
                    ...
                    if current_str == "":
                        ...
                        valid_str = True
                        is_msg_present, current_str, recent_msg = \
                            com_parser.get_messages(current_str)
                if is_msg_present:
                    # log_file.write("server::listen::parse return: " + current_str + "\n")
                    all_messages, recent_msg = utils.left_join_lists(all_messages, recent_msg)
                    if recent_msg != []:
                ...
                if (time.time() - start_time) > 3:
                    ...
                    all_messages_file.write("Period: " + str(period_of_listening) + "\n\n")
                    log_messages_to_file(all_messages_file, all_messages)
